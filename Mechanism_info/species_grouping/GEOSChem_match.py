...
def group_all_species(df_in, use, smart_groups):
    ...
    df_out['Explicit?'] = df_in['Explicit?']
    df_out['Formula'] = df_in['Formula']
    df_out['FullName'] = df_in['FullName']
    df_out['MW_g'] = df_in['MW_g']
    df_out['Epoxides']=list(np.full([len(df_out),1], np.nan)) 
    for i, species in enumerate(df_allspecies['NAME']):
        print(i, ' Species: ', df_allspecies.loc[i,'NAME']) # Print off name of what you're parsing. 
        if df_in.loc[i, 'INCHI'] is not np.nan:
            print('Initial INCHI: ', df_allspecies.loc[i,'INCHI'])
            df_allspecies_stripped = df_allspecies.loc[i,'INCHI'].strip()
            print('Stripped INCHI: ', df_allspecies_stripped)
            df_allspecies_stripped_split = re.split(r'[;\r\n]+', df_allspecies_stripped)
            print('Split INCHI: ', df_allspecies_stripped_split[0])

            df_out.at[i, 'INCHI'] = df_in.loc[i, 'INCHI']

            molec = Chem.MolFromInchi(df_allspecies_stripped_split[0], sanitize=True, removeHs=False, logLevel=None)  #Turn this molec it into an RDKit molecule object. 
            ...
            #Some species don't have InChis filled out since it isn't clear what they are. Leave those unfilled as nans so as to be clear as to why they aren't filled out for all the parameters.
            print('No InChi for ', species)
            df_out.at[i, 'INCHI'] = np.nan
    ...
    df_out.to_excel(dirpath + 'Mechanism_info/geoschem_allspecies_info_rdkit.xlsx')
    print('Saved spreadsheet for all GEOS-Chem RDKit info: ', dirpath + 'Mechanism_info/geoschem_allspecies_info_rdkit.xlsx')

    # For those that don't have InChI's, we'll need to undergo manual review to sort into groupings needed for deposition. Make separate Excel sheet to review that.
    df_manual_review = df_out[df_out['INCHI'].isna()].copy()
    df_manual_review["Original_Index"] = df_manual_review.index
    df_manual_review.reset_index(drop=True, inplace=True)
    df_manual_review.to_excel(dirpath + 'Mechanism_info/geoschem_manual_review_species_rdkit.xlsx')
...
def deposition_file_GEOSChem(den_groupings):
    ...
    with open(dirpath + 'Mechanism_info/deposition_GEOSChem_text_for_matlab_ovocs.txt', 'w', encoding="utf-8") as f:
        ...
        print('Saved text file to filepath: ', dirpath + 'Mechanism_info/deposition_GEOSChem_text_for_matlab_ovocs.txt')

### CALL FUNCTIONS ###

# path_all_species = dirpath + 'F0AM-4.3.0.1/Chem/GEOSChem/GC_database_final_UPDATED.xlsx'
# df_allspecies = pd.read_excel(path_all_species)

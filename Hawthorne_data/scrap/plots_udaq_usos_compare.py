...
def hawthorne_ozone_fill_usos(udaq_o3_filepath):
    ...
    df_alldays['Mobile Lab O3'] = df_alldays['O3_ppbv']
    ...
    df_ozone = pd.concat([df_alldays['Mobile Lab O3'], df_udaq_o3_load['UDAQ O3']], axis=1)
 
    #Combine the USOS and UDAQ measurements into merged_ozone column, preferentially filling with USOS vals
    df_ozone['merged_ozone'] = df_ozone['Mobile Lab O3'].combine_first(df_ozone['UDAQ O3'])
...
def ozone_plot_comparison_udaq_usos_merged(ozone_saved_filepath):
    ...
    #adds padding of NaNs for first day and last day of campaign, since they need to have the same length of time as the other days in order to plot
    new_start_time = pd.Timestamp('2024-07-30 00:00:00')
 ...
 #####
    ...
    ax.plot(df_all_o3_load.index, df_all_o3_load['Mobile Lab O3'], linestyle = 'solid', color='m', marker='x',label='Mobile Lab', alpha = 0.7)
    ...
    ax.xaxis.set_minor_locator(mdates.HourLocator(byhour=[0, 3, 6, 12, 18, 21]))
    ...
    plt.savefig(dirpath + '/Plotting/USOS_Campaign_analysis/plots/hawthorne_udaq_usos_ozone_comparison_0730_0801.png', dpi =300)
    plt.show()

    
    # ddd=df_all_o3_load.copy()
    # for hr in range(0,24):
    #     msk= (ddd.index.hour == hr)
    #     print()

    # m=df_all_o3_load['UDAQ O3']
    # o=df_all_o3_load['Mobile Lab O3']
    # diff=m-o
    
    # norm_MBF=np.nansum(diff)/np.nansum(m)
    # print(norm_MBF)
    ...
    df_overnight = df_all_o3_load.between_time('00:00', '05:00')  # '11:00' included
    udaq_overnight = df_overnight['UDAQ O3'].mean()
    mobile_lab_overnight= df_overnight['Mobile Lab O3'].mean()

    df_morning = df_all_o3_load.between_time('06:00', '11:00')  # '11:00' included
    udaq_morning = df_morning['UDAQ O3'].mean()
    mobile_lab_morning = df_morning['Mobile Lab O3'].mean()

    df_afternoon = df_all_o3_load.between_time('12:00', '17:00')  # '11:00' included
    udaq_afternoon = df_afternoon['UDAQ O3'].mean()
    mobile_lab_afternoon = df_afternoon['Mobile Lab O3'].mean()

    df_evening = df_all_o3_load.between_time('18:00', '23:00')  # '11:00' included
    udaq_evening = df_evening['UDAQ O3'].mean()
    mobile_lab_evening = df_evening['Mobile Lab O3'].mean()

    percent_difference_overnight = (udaq_overnight-mobile_lab_overnight)/(mobile_lab_overnight)*100
    percent_difference_morning = (udaq_morning-mobile_lab_morning)/(mobile_lab_morning)*100
    percent_difference_afternoon = (udaq_afternoon-mobile_lab_afternoon)/(mobile_lab_afternoon)*100
    percent_difference_evening = (udaq_evening-mobile_lab_evening)/(mobile_lab_evening)*100

    print('% Difference Overnight (12-5 AM):', percent_difference_morning)
    print('% Difference Morning (6-11 AM):', percent_difference_morning)
    print('% Difference Afternoon (12-5 PM):',percent_difference_afternoon)
    print('% Difference Evening (6-11 PM):',percent_difference_evening)

    df_all_o3_load['percent_diff'] = ((df_all_o3_load['UDAQ O3']-df_all_o3_load['Mobile Lab O3']) / df_all_o3_load['Mobile Lab O3'])*100
    print(df_all_o3_load['percent_diff'].values)

    from scipy.signal import find_peaks
    peaks, _ = find_peaks(df_all_o3_load['Mobile Lab O3'])
    troughs, _ = find_peaks(-df_all_o3_load['Mobile Lab O3'])
    extremes = np.sort(np.concatenate([peaks, troughs]))
    print(df_all_o3_load.index[extremes])
    for ex in df_all_o3_load.index[extremes]:
        print(df_all_o3_load['Mobile Lab O3'][ex])

    extreme_diffs = df_all_o3_load['percent_diff'].iloc[extremes]
    print(extreme_diffs)
    non_extreme_diffs = df_all_o3_load['percent_diff'].drop(df_all_o3_load.index[extremes])
    print(non_extreme_diffs)
    print("Mean % difference at peaks/troughs:", extreme_diffs.mean())
    print("Mean % difference elsewhere:", non_extreme_diffs.mean())

    #############
    # MDA8 Exceedance Comparison
    fig, ax = plt.subplots(3,1, figsize = (30,15), constrained_layout=True)
    threshold = 70

    instance_name = ['Mobile Lab', 'UDAQ', 'Merged']
    instance_varname = ['Mobile Lab O3', 'UDAQ O3', 'merged_ozone']
    for instance in range(0,3):
        ...
        ax[instance].plot(df_all_o3_load.index, df_all_o3_load[mda8_ozone_name], color='b', linewidth = 3, label = 'Non-Exceedance')
        ax[instance].plot(df_all_o3_load.index, mda8_exceedance, color='r', linewidth = 3, label = 'Exceedance')
    ...
    #plt.tight_layout()
    plt.savefig(dirpath + '/Plotting/USOS_Campaign_analysis/plots/hawthorne_udaq_usos_ozone_exceedance_comparison.png', dpi =150)
    plt.show()

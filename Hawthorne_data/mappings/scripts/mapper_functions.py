...
output_savepath = dirpath + 'Hawthorne_data/mappings/script_output/'
manual_savepath = dirpath + 'Hawthorne_data/mappings/manually_edited/'
...
def hawthorne_usos_extract_species(udaq_voc_filepath, epa_parameter_info, filename):
    ...
    excel_file = output_savepath + filename + '.xlsx'
    csv_file = output_savepath + filename + '.csv'
...
def csv_retrieve_compounds(data_filepath, parameter_column, species_column, cas_column):
    ...
    Requires the CSV output (called Hawthorne_EPA_match.csv) from the hawthorne_usos_extract_species function that matches the original UDAQ file without species name data to the EPA parameters.

    INPUTS: 
    -------  
    (1) data_filepath - STR of full path to the file from hawthorne_usos_extract_species output (Hawthorne_EPA_parameters_match.csv)
    (2) parameter_column - STR of the column name for Parameter Codes in Hawthorne_EPA_parameters_match.csv file
    (3) species_column - STR of the column name for EPA Species in Hawthorne_EPA_parameters_match.csv file
    (4) cas_column - STR of the column name for CAS Numbers in Hawthorne_EPA_parameters_match.csv file
...
def get_chem_info_epa(cmpd_list, parameters_list, cas_hawthorne_list, return_df, quiet,
    ...
    #### NOTE ####
    #AFTER RUNNING FUNCTION, MAKE SURE TO EDIT M AND P XYLENE MANUALLY AND RESAVE AS
    #manually_edited/EPA_Hawthorne_pubchem_match_updated.xlsx and csv
...
def comptox_extract(comptox_file, pubchem_match_file, save_filename):
    """
    #BEFORE RUNNING FUNCTION, MAKE SURE TO EDIT M AND P XYLENE MANUALLY AND RESAVE AS
    #manually_edited/EPA_Hawthorne_pubchem_match_updated.xlsx and csv
    Index   Parameter   EPA Species query_results       iupac_name  synonyms    cid     mw      formula inchi                                           inchi_key                   SMILES          cas_number
    45      45109	    m-Xylene	Entered manually	1,3-xylene	            7929	106.16	C8H10	InChI=1S/C8H10/c1-7-4-3-5-8(2)6-7/h3-6H,1-2H3	IVSZLXZYQVIEFR-UHFFFAOYSA-N	CC1=CC(=CC=C1)C	108-38-3
    46      45109	    p-Xylene	Entered manually	1,4-xylene	            7809	106.16	C8H10	InChI=1S/C8H10/c1-7-3-5-8(2)6-4-7/h3-6H,1-2H3	URLKBWYHVLBVBO-UHFFFAOYSA-N	CC1=CC=C(C=C1)C	106-42-3
    ...
    excel_file = output_savepath +  save_filename + '.xlsx'
    csv_file = output_savepath +   save_filename + '.csv'
...
#     epa_parameter_info = dirpath + 'Hawthorne_data/mappings/given_data/epa_parameters_official.csv',
#     filename='Hawthorne_EPA_match'
# )

# parameter_codes_list, species_list, cas_list = csv_retrieve_compounds(
#     data_filepath = output_savepath + Hawthorne_EPA_parameters_match.csv',
...
# comptox_extract(
#     comptox_file = manual_savepath + 'comptox_batch_search_all.csv',
#     pubchem_match_file = manual_savepath + 'EPA_Hawthorne_pubchem_match_updated.csv',
#     save_filename= 'EPA_CRACMM_mapped'     
# )

geoschem_mech_mapping(
    CRACMM_mapped_file = output_savepath+'EPA_CRACMM_mapped.csv',
    geoschem_data_file = dirpath + 'F0AM-4.3.0.1/Chem/GEOSChem/GC_database_final_UPDATED.csv',
    save_filename = 'UDAQ_Hawthorne_CRACMM_GEOSCHEM_mapped'
)

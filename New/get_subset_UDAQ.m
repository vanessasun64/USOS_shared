function [out]= get_subset_UDAQ(t_start, t_end)
% Function to get a subset of the data structure based on time, date,
    ...
    dirpath2UDAQ_win= '\Users\u1545774\Documents\GitHub\USOS_shared\F0AM-4.3.0.1\Campaign_Data\matlab_merge\udaq\corrected\';
    UDAQfilename_win = '20240719_20240721_1hr_udaq_vocs_struct_for_MATLAB.mat';
    fullpath2UDAQ_win = fullfile('C:',dirpath2UDAQ_win,UDAQfilename_win);
    load(fullpath2UDAQ_win); % loads struct named USOS. 
    ...
    % & not do this again: 
    %USOS.time_UTC= datetime(UDAQ.time_UTC./1e9, 'ConvertFrom', 'posixtime');
    UDAQ.time_MST= datetime(UDAQ.time_local./1e9, 'ConvertFrom', 'posixtime');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ideal=(t_start:minutes(60):t_end);

    % Get Indices in data of the time window we've sub-selected data for. 
    cond=find(UDAQ.time_MST>= t_start & UDAQ.time_MST <= t_end); 
    ...
    if length(cond) == length(ideal)
        ...
        FieldList = fieldnames(UDAQ);
        for iField = 1:numel(FieldList)
           Field    = FieldList{iField};
           if length(UDAQ.(Field))==length(UDAQ.time_MST)
              out.(Field) = UDAQ.(Field)(cond);
           else 
              out.(Field) = UDAQ.(Field);
           end
        end
